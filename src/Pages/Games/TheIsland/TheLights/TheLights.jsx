...
const handleToggleNo = function() {
    ...
    return ( 
        ...
        <div className='games'>
            {(toggleNo === false)?<button className='no'><GiTrail onClick={handleToggleNo} size='4rem'/></button>:<No/>}
            <p>===</p><GiShrug size='4rem'/><p>===</p>
            {(toggleYes === false)?<button className='yes'><BsStars onClick={handleToggleYes} size='4rem'/></button>:<Yes/>}
        </div>
        </div>
     );
}
